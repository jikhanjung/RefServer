...
RefServer는 학술 논문 PDF 파일을 업로드하면 **하이브리드 언어 감지 OCR**, 품질 평가, 임베딩 생성, 레이아웃 분석, 메타데이터 추출을 자동으로 수행하는 통합 지능형 시스템입니다. **v0.1.11에서 세계 최고 수준의 다국어 OCR 언어 감지 시스템을 구현**하여 텍스트 레이어 없는 PDF에서도 완벽한 언어 인식과 최적 OCR 품질을 보장합니다.

## 🎉 **v0.1.11 구현 완료 상태**
- ✅ **하이브리드 언어 감지 OCR** (텍스트 → LLaVA → 다중 OCR 샘플링)
- ✅ **완벽한 다국어 지원** (한국어/일본어/중국어 등 10개 언어 자동 감지)
- ✅ **LLaVA 시각적 언어 감지** (GPU 환경에서 이미지 기반 언어 인식)
- ✅ **다중 OCR 샘플링** (7개 언어 동시 테스트 후 최적 선택)
...
### 🔍 **v0.1.11 혁신: 하이브리드 언어 감지 OCR**
- **🌐 완벽한 다국어 지원**: 텍스트 레이어 없는 PDF에서도 정확한 언어 감지
  - **Level 1**: 텍스트 기반 감지 (기존 텍스트 50자 이상)
  - **Level 2**: LLaVA 시각적 언어 감지 (GPU 환경, 문자 모양 분석)
  - **Level 3**: 다중 OCR 샘플링 (7개 언어 테스트 후 최적 선택)
- **🎯 스마트 OCR 품질**: 복합 점수로 최적 언어 선택 (OCR 신뢰도 + 텍스트 길이 + 언어 매칭)
- **⚡ 고성능 처리**: 샘플 영역 추출 + 200 DPI + 우선순위 언어로 효율적 감지
- **🔧 완전 자동화**: 사용자 개입 없이 한국어/일본어/중국어 등 완벽 지원
